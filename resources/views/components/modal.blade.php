...
<div x-data="{ show: @entangle($attributes->wire('model')).live }" x-on:close.stop="show = false; {{ $onclose }}"
  ...
  <template x-teleport="body">
    <div x-show="show" class="jetstream-modal fixed inset-0 flex min-h-full items-center justify-center overflow-y-auto p-3 sm:p-4 my-auto" style="display: none; z-index: {{ $baseZIndex }};">
      <!-- Backdrop -->
      <div x-show="show"
        class="fixed inset-0 bg-gray-900/60 dark:bg-gray-950/80 backdrop-blur-md transition-opacity"
      ...
      <!-- Modal Dialog Card -->
      <div x-show="show"
        class="w-full {{ $maxWidth }} transform overflow-hidden rounded-2xl bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border border-white/60 dark:border-gray-700/60 shadow-2xl transition-all sm:mx-auto max-h-[82vh] sm:max-h-[88vh] flex flex-col my-auto"
