  ...
  @if(request()->routeIs('payroll.*') || (auth()->user()->isPayroll && !request()->routeIs('hr.*') && !request()->routeIs('home') && !request()->routeIs('attendance-history') && !request()->routeIs('user.*')))
    ...
    <nav class="fixed bottom-0 left-0 right-0 z-50 block md:hidden border-t border-gray-200/90 dark:border-gray-800 bg-white/95 dark:bg-gray-900/95 backdrop-blur-lg shadow-lg select-none" style="padding-bottom: env(safe-area-inset-bottom, 0px);">
      <div class="flex items-center h-16 max-w-md mx-auto relative px-1">
        ...
        <a href="{{ route('payroll.dashboard') }}" class="group relative flex flex-1 flex-col items-center justify-center h-full py-1 text-center transition-colors duration-150 {{ $active ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300' }}">
        ...
        <a href="{{ route('payroll.history') }}" class="group relative flex flex-1 flex-col items-center justify-center h-full py-1 text-center transition-colors duration-150 {{ $active ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300' }}">
          <span class="absolute top-0 h-0.5 w-8 rounded-full bg-blue-600 dark:bg-blue-400 transition-all duration-200 origin-center {{ $active ? 'opacity-100 scale-x-100' : 'opacity-0 scale-x-0' }}"></span>
          <div class="h-6 w-6 flex items-center justify-center mb-0.5">
            @if($active)
          ...
          <span class="text-[11px] leading-none font-bold tracking-tight">Payroll</span>
        </a>
        ...
        <a href="{{ route('payroll.saving-transactions') }}" class="group relative flex flex-1 flex-col items-center justify-center h-full py-1 text-center transition-colors duration-150 {{ $active ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300' }}">
          <span class="absolute top-0 h-0.5 w-8 rounded-full bg-blue-600 dark:bg-blue-400 transition-all duration-200 origin-center {{ $active ? 'opacity-100 scale-x-100' : 'opacity-0 scale-x-0' }}"></span>
          <div class="h-6 w-6 flex items-center justify-center mb-0.5">
            @if($active)
              <x-heroicon-s-building-library class="h-5 w-5 text-blue-600 dark:text-blue-400" />
            @else
          ...
          <span class="text-[11px] leading-none font-bold tracking-tight">Syirkah</span>
        </a>
        ...
        <a href="{{ route('payroll.employee-salaries') }}" class="group relative flex flex-1 flex-col items-center justify-center h-full py-1 text-center transition-colors duration-150 {{ $active ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300' }}">
          <span class="absolute top-0 h-0.5 w-8 rounded-full bg-blue-600 dark:bg-blue-400 transition-all duration-200 origin-center {{ $active ? 'opacity-100 scale-x-100' : 'opacity-0 scale-x-0' }}"></span>
          <div class="h-6 w-6 flex items-center justify-center mb-0.5">
            @if($active)
              <x-heroicon-s-calculator class="h-5 w-5 text-blue-600 dark:text-blue-400" />
            @else
          ...
          <span class="text-[11px] leading-none font-bold tracking-tight whitespace-nowrap">Master Gaji</span>
        </a>

        <!-- 5. PINJAMAN (LOANS) -->
        @php $active = request()->routeIs('payroll.loans'); @endphp
        <a href="{{ route('payroll.loans') }}" class="group relative flex flex-1 flex-col items-center justify-center h-full py-1 text-center transition-colors duration-150 {{ $active ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300' }}">
          <span class="absolute top-0 h-0.5 w-8 rounded-full bg-blue-600 dark:bg-blue-400 transition-all duration-200 origin-center {{ $active ? 'opacity-100 scale-x-100' : 'opacity-0 scale-x-0' }}"></span>
          <div class="h-6 w-6 flex items-center justify-center mb-0.5">
            @if($active)
              <x-heroicon-s-hand-raised class="h-5 w-5 text-blue-600 dark:text-blue-400" />
            @else
          ...
          <span class="text-[11px] leading-none font-bold tracking-tight">Pinjaman</span>
        </a>
      </div>
    </nav>
