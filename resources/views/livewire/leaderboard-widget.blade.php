<div class="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-8"
  ...
  <div class="flex flex-col justify-between p-0 border-0 bg-transparent sm:rounded-2xl sm:border sm:border-gray-100 sm:bg-gray-50/50 sm:p-5 dark:sm:border-gray-700/60 dark:sm:bg-gray-800/40">
    <div>
      <div class="mb-4 flex flex-wrap items-center justify-between gap-2 border-b border-gray-200/80 pb-3 sm:mb-5 sm:pb-4 dark:border-gray-700">
        ...
        <span class="inline-flex items-center rounded-full bg-indigo-50 px-2.5 py-0.5 text-xs font-semibold text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300 sm:px-3 sm:py-1">
          • Real-time Updated
        </span>
      </div>
      ...
      @if ($globalTopEmployees->isNotEmpty())
        <div class="flex flex-col gap-2.5 sm:gap-3">
          @foreach ($globalTopEmployees as $index => $st)
            ...
            <div class="flex items-center justify-between rounded-xl sm:rounded-2xl border px-3 py-2.5 sm:px-4 sm:py-3.5 transition-all duration-200 hover:shadow-xs {{ $cardBg }}">
              ...
              <div class="flex items-center gap-2.5 sm:gap-3.5 min-w-0 flex-1 mr-2">
                ...
                <div class="flex h-8 w-8 sm:h-10 sm:w-10 shrink-0 items-center justify-center">
                  @if ($rank === 1)
                    <div class="flex h-7 w-7 sm:h-9 sm:w-9 items-center justify-center rounded-full bg-gradient-to-br from-amber-300 via-amber-400 to-yellow-500 p-1 text-amber-950 shadow-md shadow-amber-400/30 ring-2 ring-amber-200 dark:ring-amber-500/40">
                  ...
                  @elseif ($rank === 2)
                    <div class="flex h-7 w-7 sm:h-9 sm:w-9 items-center justify-center rounded-full bg-gradient-to-br from-slate-200 via-slate-300 to-slate-400 p-1 text-slate-800 shadow-md shadow-slate-300/30 ring-2 ring-slate-200 dark:ring-slate-600">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-4 w-4 sm:h-5 sm:w-5">
                        <path fill-rule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z" clip-rule="evenodd" />
                      </svg>
                    </div>
                  @elseif ($rank === 3)
                    <div class="flex h-7 w-7 sm:h-9 sm:w-9 items-center justify-center rounded-full bg-gradient-to-br from-amber-700 via-amber-800 to-amber-900 p-1 text-amber-100 shadow-md shadow-amber-900/30 ring-2 ring-amber-700/50">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-4 w-4 sm:h-5 sm:w-5">
                        <path fill-rule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z" clip-rule="evenodd" />
                      </svg>
                    </div>
                  @else
                    <div class="flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-full border border-gray-200 bg-gray-50 text-xs sm:text-sm font-semibold text-gray-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400">
                      {{ $rank }}
                    </div>
                  @endif
                </div>
                ...
                <!-- Profile Avatar -->
                <div class="h-9 w-9 sm:h-11 sm:w-11 shrink-0 overflow-hidden rounded-xl sm:rounded-2xl bg-indigo-100 ring-1 ring-gray-200 dark:bg-indigo-950 dark:ring-gray-700">
                  @if ($st->user?->profile_photo_url)
                    <img src="{{ $st->user->profile_photo_url }}" alt="{{ $st->user->name }}" class="h-full w-full object-cover">
                  @else
                    <div class="flex h-full w-full items-center justify-center text-xs sm:text-sm font-bold text-indigo-600 dark:text-indigo-300">
                ...
                <div class="flex flex-col min-w-0 flex-1">
                  <span class="text-xs sm:text-sm font-bold text-gray-900 dark:text-white truncate">
                    {{ $st->user?->name ?? 'Karyawan' }}
                  </span>
                  <span class="text-[11px] sm:text-xs font-medium text-amber-600 dark:text-amber-400 truncate">
              ...
              <div class="shrink-0">
                <div class="inline-flex items-center rounded-full border border-emerald-300 bg-emerald-50/90 px-2.5 py-1 text-xs font-bold text-emerald-800 shadow-2xs dark:border-emerald-700/80 dark:bg-emerald-950/80 dark:text-emerald-200 sm:px-3.5">
                  <span class="mr-1 text-emerald-600 dark:text-emerald-400">•</span> {{ number_format($st->score, 0) }} Pts
                </div>
              </div>
            </div>
          @endforeach
  ...
  <div class="flex flex-col justify-between p-0 border-0 bg-transparent sm:rounded-2xl sm:border sm:border-gray-100 sm:bg-gray-50/50 sm:p-5 dark:sm:border-gray-700/60 dark:sm:bg-gray-800/40">
    <div>
      <div class="mb-4 flex flex-wrap items-center justify-between gap-2 border-b border-gray-200/80 pb-3 sm:mb-5 sm:pb-4 dark:border-gray-700">
        ...
        <span class="inline-flex items-center rounded-full bg-sky-50 px-2.5 py-0.5 text-xs font-semibold text-sky-700 dark:bg-sky-900/50 dark:text-sky-300 sm:px-3 sm:py-1">
          • Divisi Scoped
        </span>
      </div>
