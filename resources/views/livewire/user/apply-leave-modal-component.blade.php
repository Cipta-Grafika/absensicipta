        ...
        <x-slot name="content">
            <form wire:submit.prevent="submit" id="applyLeaveForm">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="{{ $status === 'imp' ? 'sm:col-span-1' : 'sm:col-span-2' }}">
                    ...
                    <div class="sm:col-span-1">
                        <x-label for="from">
                            <span>{{ $status === 'imp' ? 'Tanggal IMP' : 'Tanggal mulai' }}</span>
                        </x-label>
                        <x-input type="date" id="from" wire:model="from"
                            min="{{ $status === 'imp' ? date('Y-m-01') : date('Y-m-d') }}"
                            max="{{ $status === 'imp' ? date('Y-m-t') : '' }}"
